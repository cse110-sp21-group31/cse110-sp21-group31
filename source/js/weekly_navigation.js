...
function populate(key) {
    ...
    for (let i = 0; i < 7; i += 1) {
        ...
        for (let j = 0; j < allEve.length; j += 1) {
            ...
            const start_time_minutes = start_time.substr(
                start_time.indexOf(':') + 1,
                3
            );

            const end_time_hr = end_time.slice(0, end_time.indexOf(':'));
            const end_time_minutes = end_time.substr(
                end_time.indexOf(':') + 1,
                3
            );
            ...
            console.log(
                `start time hr:min: ${start_time_hr}, ${start_time_minutes} ${start_time_ampm} end time hr:min${end_time_hr}, ${end_time_minutes} ${end_time_ampm}`
            );
            ...
            */
            ...
            if (end_time_hr < 10 && end_time_ampm == 'AM') {
                ...
                bottom =
                    20 + 10 * (10 - end_time_hr) + 10 * (end_time_minutes / 60);
                bottom = '-' + bottom + '%';
                event_label.style.bottom = bottom;
            }
